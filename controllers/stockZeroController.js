...
function normalizePagination(query) {
  ...
  const limit = Math.min(Math.max(parseInt(query.limit || '12', 10), 1), 60);
...
exports.listStockZeroEvents = async (req, res) => {
  try {
    ...
    const paginatedProducts = products.slice(safeOffset, safeOffset + limit);
